      ...
      extractSettingsInformation(keys) {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (config) {
            ...
            ensurePath(result, path3)[path3[path3.length - 1]] = configuration_1.toJSONObject(config);
          }
        }
        return result;
      }
    ...
    var SemanticTokensFeature = class extends client_1.TextDocumentFeature {
      ...
      registerLanguageProvider(options) {
        ...
        if (documentProvider !== void 0) {
          disposables.push(vscode10.languages.registerDocumentSemanticTokensProvider(options.documentSelector, documentProvider, legend));
        }
        if (rangeProvider !== void 0) {
          disposables.push(vscode10.languages.registerDocumentRangeSemanticTokensProvider(options.documentSelector, rangeProvider, legend));
        }
...
async function getDenoCommand() {
  ...
  if (!command || !workspaceFolders) {
    ...
    const list = [];
    for (const workspace6 of workspaceFolders) {
      const dir = path.resolve(workspace6.uri.fsPath, command);
      try {
...
async function buildDenoTask(target, definition, name, args, problemMatchers) {
  const exec = new vscode2.ProcessExecution(await getDenoCommand(), args, definition);
  ...
  async provideTasks() {
    ...
    for (const workspaceFolder of (_a = vscode2.workspace.workspaceFolders) != null ? _a : []) {
      for (const { command, group, problemMatchers } of defs) {
        const task = await buildDenoTask(workspaceFolder, { type: TASK_TYPE, command }, command, [command], problemMatchers);
        task.group = group;
        tasks4.push(task);
      }
    }
  ...
  constructor(panel, extensionUri) {
    ...
    __privateGet(this, _panel).webview.onDidReceiveMessage((message) => {
      switch (message.command) {
        ...
        case "init": {
          vscode3.commands.executeCommand("deno.initializeWorkspace");
          return;
        }
      }
...
function initializeWorkspace(_context, _extensionContext) {
  return async () => {
    try {
      ...
      const config = vscode5.workspace.getConfiguration(EXTENSION_NS);
      await config.update("enable", true);
      await config.update("lint", settings.lint);
      await config.update("unstable", settings.unstable);
      await vscode5.window.showInformationMessage("Deno is now setup in this workspace.");
    } catch {
      vscode5.window.showErrorMessage("Deno project initialization failed.");
    }
  };
}
...
function startLanguageServer(context, extensionContext2) {
  return async () => {
    ...
    if (extensionContext2.client) {
      ...
      vscode5.commands.executeCommand("setContext", ENABLEMENT_FLAG, false);
      await client2.stop();
    }
    ...
    const client = new import_node.LanguageClient(LANGUAGE_CLIENT_ID, LANGUAGE_CLIENT_NAME, serverOptions, extensionContext2.clientOptions);
    context.subscriptions.push(client.start());
    await client.onReady();
    extensionContext2.client = client;
    vscode5.commands.executeCommand("setContext", ENABLEMENT_FLAG, true);
    extensionContext2.serverInfo = new DenoServerInfo((_a = client.initializeResult) == null ? void 0 : _a.serverInfo);
    extensionContext2.statusBar.refresh(extensionContext2);
...
function notifyServerSemver(serverVersion) {
  return vscode5.window.showWarningMessage(`The version of Deno language server ("${serverVersion}") does not meet the requirements of the extension ("${SERVER_SEMVER}"). Please update Deno and restart.`, "OK");
}
...
function showReferences(_content, extensionContext2) {
  return (uri, position, locations) => {
    ...
    vscode5.commands.executeCommand("editor.action.showReferences", vscode5.Uri.parse(uri), extensionContext2.client.protocol2CodeConverter.asPosition(position), locations.map(extensionContext2.client.protocol2CodeConverter.asLocation));
  };
}
function status(_context, _extensionContext) {
  return () => {
    const uri = vscode5.Uri.parse("deno:/status.md");
    return vscode5.commands.executeCommand("markdown.showPreviewToSide", uri);
  };
}
function test(_context, _extensionContext) {
  return async (uriStr, name) => {
    var _a;
    const uri = vscode5.Uri.parse(uriStr, true);
    const path3 = uri.fsPath;
    const config = vscode5.workspace.getConfiguration(EXTENSION_NS, uri);
    const testArgs = [
    ...
    const args = ["test", ...testArgs, "--filter", name, path3];
    const definition = {
      type: TASK_TYPE,
      command: "test",
      args,
    ...
    assert(vscode5.workspace.workspaceFolders);
    const target = vscode5.workspace.workspaceFolders[0];
    const task = await buildDenoTask(target, definition, `test "${name}"`, args, ["$deno-test"]);
    ...
    return vscode5.tasks.executeTask(task);
  };
}
...
var vscode6 = __toModule(require("vscode"));
...
var DenoDebugConfigurationProvider = class {
  ...
  async resolveDebugConfiguration(workspace6, config) {
    if (!config.type && !config.request && !config.name) {
      ...
      if (editor && (langId === "typescript" || langId === "javascript" || langId === "typescriptreact" || langId === "javascriptreact")) {
        vscode6.debug.startDebugging(workspace6, {
          request: "launch",
          name: "Launch Program",
          type: "pwa-node",
          program: "${file}",
    ...
    if (!config.program) {
      await vscode6.window.showErrorMessage("Cannot resolve a program to debug");
      return void 0;
    }
    return config;
  }
...
var vscode7 = __toModule(require("vscode"));
var _inner;
var DenoStatusBar = class {
  constructor() {
    __privateAdd(this, _inner, void 0);
    __privateSet(this, _inner, vscode7.window.createStatusBarItem(vscode7.StatusBarAlignment.Right, 0));
  }
  dispose() {
...
var vscode8 = __toModule(require("vscode"));
function getTsApi(getPluginSettings) {
  let api;
  (async () => {
    try {
      const extension = vscode8.extensions.getExtension(TS_LANGUAGE_FEATURES_EXTENSION);
      ...
      await vscode8.window.showErrorMessage(msg);
    }
  })();
  return {
    refresh() {
...
var vscode9 = __toModule(require("vscode"));
var LANGUAGES = [
  "typescript",
  "javascript",
  "typescriptreact",
...
function configToWorkspaceSettings(config) {
  ...
  for (const key of workspaceSettingsKeys) {
    ...
    assert(value);
    workspaceSettings[key] = (_c = (_b = (_a = value.workspaceLanguageValue) != null ? _a : value.workspaceValue) != null ? _b : value.globalValue) != null ? _c : value.defaultValue;
  }
  for (const key of resourceSettingsKeys) {
    const value = config.inspect(key);
    assert(value);
    workspaceSettings[key] = (_f = (_e = (_d = value.workspaceLanguageValue) != null ? _d : value.workspaceValue) != null ? _e : value.globalValue) != null ? _f : value.defaultValue;
  }
...
function getWorkspaceSettings() {
  const config = vscode9.workspace.getConfiguration(EXTENSION_NS);
  ...
  if (event.affectsConfiguration(EXTENSION_NS)) {
    ...
    for (const [key, { scope }] of Object.entries(extensionContext.documentSettings)) {
      extensionContext.documentSettings[key] = {
        scope,
        settings: configToResourceSettings(vscode9.workspace.getConfiguration(EXTENSION_NS, scope))
      };
    }
    extensionContext.tsApi.refresh();
    extensionContext.statusBar.refresh(extensionContext);
    if (event.affectsConfiguration("deno.path")) {
      vscode9.commands.executeCommand("deno.restart");
    }
  }
}
function handleDocumentOpen(...documents) {
  ...
  for (const doc of documents) {
    ...
    extensionContext.documentSettings[path2.normalize(doc.uri.fsPath)] = {
      scope: { languageId, uri },
      settings: configToResourceSettings(vscode9.workspace.getConfiguration(EXTENSION_NS, { languageId, uri }))
    };
    didChange = true;
  }
  if (didChange) {
...
async function activate(context) {
  ...
  vscode9.workspace.onDidOpenTextDocument(handleDocumentOpen, extensionContext, context.subscriptions);
  vscode9.workspace.onDidChangeConfiguration(handleConfigurationChange, extensionContext, context.subscriptions);
  extensionContext.statusBar = new DenoStatusBar();
  context.subscriptions.push(extensionContext.statusBar);
  context.subscriptions.push(vscode9.workspace.registerTextDocumentContentProvider(SCHEME, new DenoTextDocumentContentProvider(extensionContext)));
  context.subscriptions.push(vscode9.debug.registerDebugConfigurationProvider("deno", new DenoDebugConfigurationProvider(getWorkspaceSettings)));
  ...
  extensionContext.workspaceSettings = getWorkspaceSettings();
  handleDocumentOpen(...vscode9.workspace.textDocuments);
...
function deactivate() {
  ...
  extensionContext.statusBar.refresh(extensionContext);
  vscode9.commands.executeCommand("setContext", ENABLEMENT_FLAG, false);
  return client.stop();
}
function createRegisterCommand(context) {
  return function registerCommand(name, factory) {
    ...
    context.subscriptions.push(vscode9.commands.registerCommand(fullName, command));
  };
}
